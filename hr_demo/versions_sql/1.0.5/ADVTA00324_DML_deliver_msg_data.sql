...
update  DELIVER_MSG set MSG_TXT = 'Traveler cannot be on leave during the travel period',MSG_EXPL = 'The specified traveler must not be on leave during the trip''s start and begin dates. ',MSG_TXT_UP = 'TRAVELER CANNOT BE ON LEAVE DURING THE TRAVEL PERIOD',
MSG_SEV = 1 where MSG_CD = 'A7830'
GO

update DELIVER_MSG set MSG_TXT = 'Traveler must not be on leave during the travel period.',MSG_EXPL = 'The specified traveler must not be on leave during the trip''s start and begin dates.' , MSG_TXT_UP = 'TRAVELER MUST NOT BE ON LEAVE DURING THE TRAVEL PERIOD.',
...
INSERT INTO DELIVER_MSG (MSG_CD,MSG_TXT,MSG_SEV,MSG_EXPL,OV_LVL,AMS_ROW_VERS_NO,MSG_TXT_UP,TBL_LAST_DT) VALUES ('A3449','A reason for modification must be entered on the document to record the reasoning behind revising the original entry.',2,'A reason for modification must be entered on the document to record the reasoning behind revising the original entry.',0,1,'A REASON FOR MODIFICATION MUST BE ENTERED ON THE DOCUMENT TO RECORD THE REASONING BEHIND REVISING THE ORIGINAL ENTRY.',NULL)
