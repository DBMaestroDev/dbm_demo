...
def upgrade_environment(pipe, env_num){
  ...
  if(approver != "none"){
	...
	//for(env in pair){
		echo "Executing: ${pipe["java_cmd"]} -Upgrade -ProjectName ${pipe["pipeline"]} -EnvName ${environment} -PackageName ${version} -Server ${pipe["server"]} ${pipe["credential"]}"
		bat "${pipe["java_cmd"]} -Upgrade -ProjectName ${pipe["pipeline"]} -EnvName ${environment} -PackageName ${version} -Server ${pipe["server"]} ${pipe["credential"]}"
  //}
}
  ...
  if(!env.Skip_Packaging || env.Skip_Packaging == "No"){
    ...
    pipe["base_schema"].split(",").each {schema ->
      ...
      if(pipe["file_strategy"] == "version"){
        ...
        echo "# Copying Version Directory"
		echo "from ${source_dir}${sep()}${version} to ${version_dir}"
  		bat "xcopy /s /y /i \"${source_dir}${sep()}${version}\" \"${version_dir}\""
      }else{
        // This is to separate scripts into schema
        def processed_dir = "${source_dir}${sep()}processed${sep()}${v_version}"
        ensure_dir(processed_dir)
        echo "# Copying Prefixed Files into Directory"
  		def filter = ""
...
def execute(pipe_info, env_num){
  stage(pipe_info["environments"][env_num]){
	  ...
	  println "Executing Upgrade Command"
	  node(pipe_info["dbm_node"]){
